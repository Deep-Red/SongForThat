...
def seed_releases(start_release, finish_release)
  @start_line = start_release * 5
  @finish_line = (finish_release + 1) * 5
  @line_count = 0
    ...
    if @line_count > @start_line && @line_count <= @finish_line
      ...
      when 4
        ...
        unless Rails.env.production?
          puts "Processed release ##{@line_count / 5} in #{Time.now - @start_time}" if @line_count % 10000 == 0
        end
      else
        raise "line_count (#{@line_count}) % 5 not in 0..4"
      end
    elsif @line_count >= @finish_line
      puts "Processed releases #{start_release} through #{finish_release}!"
      return
    end
  end
